...
/**
 * 
 * @author Christian Lisle This class contains game mechanics for Space Invaders
 */
public class Tableau {
	...
	private boolean checkForEnemyHit(DefenderProjectile d) {
		for (int y = 0; y < enemyFleet.length; y++) {
			for (int x = 0; x < enemyFleet[0].length; x++) {
	...
	private SpaceShip getRightMostEnemy() {
		for (int x = enemyFleet[0].length - 1; x >= 0; x--) {
			for (int y = enemyFleet.length - 1; y >= 0; y--) {
				if (enemyFleet[y][x] != null) {
	...
	private SpaceShip getLowestEnemy() {
		/*
		 * Index starts at the last position of the 2d array and returns the first index
		 * that is not null. If entire array is null, returns null.
		 */
		for (int y = enemyFleet.length - 1; y >= 0; y--) {
			for (int x = enemyFleet[0].length - 1; x >= 0; x--) {
				if (enemyFleet[y][x] != null) {
		...
		if (!gameIsOver()) {
			if (!fleetMovePositiveX) {
				if (getLeftMostEnemy().getPosition().getX() - FLEET_MOVE_X <= 0) {
					...
					translateEnemyFleet(0 - FLEET_MOVE_X, 0);
				}
			} else {
